...
export default function MembersForSelected() {
  ...
  const handleDelete = async (memberId: number) => {
    try {
      ...
      if (response.status === 200) {
        toast.success("تم حذف المشترك من النشرة البريدية بنجاح");
        setSelectCurrentData("DefaultData");
      ...
      <div className="w-full min-h-screen p-6 max-md:p-2 ">
        ...
        >
          <div className="flex items-center w-full gap-2 max-md:flex-col ">
            <div className="relative bg-white rounded-md shadow-md h-10 flex items-center justify-center w-full">
              <LuSearch
                    ...
                    <td colSpan={4}>
                      {users.length === 0 ? (
                        <div className="min-h-[70vh] flex items-center justify-center">
                          لا يوجد مستخدمين متاحين للاختيار
                        </div>
                      ) : (
